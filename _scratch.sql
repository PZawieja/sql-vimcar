SELECT * FROM dwh_main.dim_cb_subscription WHERE cb_subscr_id = '30008992141';
SELECT * FROM dwh_main.dim_cb_subscription WHERE cb_subscr_customer_id = '30003301581';
SELECT * FROM dwh_main.dim_combined_subscription WHERE customer_id = '30014401050'
SELECT * FROM dwh_main.dim_combined_invoice_line WHERE customer_id = '30014401050';
SELECT * FROM dwh_main.dim_cb_subscription WHERE cb_subscr_current_term_end_ts is NULL-- AND cb_subscr_status = 'non_renewing'
SELECT DISTINCT cb_subscr_status FROM dwh_main.dim_cb_subscription WHERE cb_subscr_next_billing_ts is NULL-- AND cb_subscr_status = 'non_renewing'
SELECT * FROM dwh_main.dim_combined_subscription ORDER BY subscription_end_dt DESC ;
SELECT * FROM dwh_main.umd_uk_invoices_not_imported_to_chargebee
--------------------------------------------------------------------------------
        ...
        s.cb_subscr_customer_id                                                                               AS customer_id
                  ...
                  , CASE
                        WHEN s.cb_subscr_status IN ('non_renewing','cancelled')
                            THEN coalesce(e.event_subscr_cancelled_ts, ctr.contract_canceled_date_key,
                                          s.cb_subscr_cancelled_ts, s.cb_subscr_current_term_start_ts, s.updated_at) AT TIME ZONE 'Europe/Berlin'
        END::DATE AS subscription_cancelled_dt
                  , lower(btrim(
            CASE
                WHEN s.cb_subscr_status IN ('non_renewing','cancelled')
                    THEN coalesce(s.cb_subscr_cf_cancellation_reason, s.cb_subscr_cancel_reason_code, ctr.contract_cancelation_reason, 'n/a')
                END)) AS subscription_cancellation_reason
                  , (coalesce(CASE
                                  WHEN s.cb_subscr_status IN ('cancelled', 'non_renewing')
                                      THEN coalesce(s.cb_subscr_current_term_end_ts, s.cb_subscr_cancelled_ts)
                                  WHEN s.cb_subscr_status = 'future'
                                      THEN s.cb_subscr_next_billing_ts + (p.cb_plan_duration_months * INTERVAL '1 month')
                                  WHEN coalesce(s.cb_subscr_plan_id,'dummy') NOT LIKE '%3_y%'
                                        AND s.cb_subscr_billing_period_unit = 'year'
                                      THEN cb_subscr_next_billing_ts
                  ...
                  , CASE
                      WHEN s.cb_subscr_plan_id LIKE '%3_y%'
                        THEN  (CASE
                                  WHEN s.cb_subscr_status IN ('cancelled', 'non_renewing')
                                      THEN coalesce(s.cb_subscr_current_term_end_ts, s.cb_subscr_cancelled_ts)
                                  WHEN s.cb_subscr_status = 'future'
                                  ...
                                  WHEN s.cb_subscr_remaining_billing_cycles > 0
                                        AND s.cb_subscr_billing_period_unit = 'year'
                                      THEN cb_subscr_current_term_end_ts + (s.cb_subscr_remaining_billing_cycles * INTERVAL '1 year')
                                  END AT TIME ZONE 'Europe/Berlin')::DATE
                      END AS subscription_3y_end_dt
                  , CASE
                ...
                END AS subscription_has_recurring_product -- exclude 1 time purchases, non-recurring ones
                  , CASE WHEN s.cb_subscr_plan_id LIKE '%3_y%' THEN TRUE ELSE FALSE END AS is_3y_plan
...
-- UK invoices not imported into CB: https://docs.google.com/spreadsheets/u/1/d/14PAZ_eMNi8WAAkxC7rjbqBMsYFEFt0jbEtUfLqozi1k/edit#gid=0
    SELECT
        u.customer_id
         ...
         , NULL AS subscription_3y_end_dt
    ...
    SELECT subscription_id
         ...
         , CASE
               WHEN is_3y_plan = TRUE
                        AND subscription_end_dt - INTERVAL '3 years' >= current_date
                   THEN subscription_end_dt - INTERVAL '3 years'
               WHEN is_3y_plan = TRUE
                        AND subscription_end_dt - INTERVAL '2 years' >= current_date
                   THEN subscription_end_dt - INTERVAL '2 years'
               WHEN is_3y_plan = TRUE
                        AND subscription_end_dt - INTERVAL '1 year' >= current_date
                   THEN subscription_end_dt - INTERVAL '1 year'
               ELSE subscription_end_dt
        END::DATE AS subscription_end_dt
         , subscription_3y_end_dt
...
--      , string_agg(DISTINCT subscription_id_cb::TEXT, ',') AS cb_subscription_id
    ...
    GROUP BY subscription_id, customer_id, subscription_status, subscription_start_dt, subscription_coupon_1, subscription_coupon_2,
             subscription_cancelled_dt, subscription_cancellation_reason, subscription_end_dt, is_3y_plan, subscription_3y_end_dt
)
--------------------------------------------------------------------------------
    ...
    SELECT
        ...
        END                                                                                    AS subscription_status
         ...
         , coalesce(
                    ...
                    END::DATE) FILTER ( WHERE contract_status_map = 2 AND valid_to_dt_contract_lvl >= current_date ),
                    valid_to_dt_contract_lvl, '1999-01-01') AS tmp_subscription_end_dt

         -- 3y contracts:
         , coalesce(
                    MIN(CASE WHEN three_year_contract = TRUE
                                    AND is_recurring_product = TRUE
                                    AND monthly_payment = FALSE
                ...
                END::DATE) FILTER ( WHERE contract_status_map = 2 AND valid_to_dt_contract_lvl >= current_date ),
                    valid_to_dt_contract_lvl, '1999-01-01') AS tmp_3y_subscription_end_dt

         , valid_to_dt_contract_lvl
         , true = ANY(ARRAY_AGG(is_recurring_product = TRUE)) AS subscription_has_recurring_product
    FROM cte_1 c1
...
-- SELECT * FROM cte_2 WHERE customer_id = 'K30289455' ;
   , shop_subscriptions AS (
    SELECT
        subscription_id
         ...
         , CASE
               WHEN subscription_status = 'cancelled'
                       ...
                       OR subscription_has_recurring_product = FALSE
                   THEN valid_to_dt_contract_lvl
        ...
        END::DATE AS subscription_end_dt
        , CASE
            WHEN three_year_ctr.contract_id IS NOT NULL
                    AND subscription_start_dt > current_date - INTERVAL '3 years'
                THEN tmp_3y_subscription_end_dt
            WHEN three_year_ctr.contract_id IS NOT NULL
                THEN valid_to_dt_contract_lvl
         END AS subscription_3y_end_dt
    ...
    FROM cte_2 c2
             ...
             LEFT JOIN (SELECT DISTINCT contract_id
                        FROM cte_1
                        WHERE three_year_contract = TRUE) AS three_year_ctr
                       ON three_year_ctr.contract_id = c2.subscription_id
)
   , cte_3 AS (
...
SELECT
    ...
    END AS subscription_cancelled_dt
     , CASE
           ...
           WHEN subscription_cancellation_reason LIKE '%storno%'
               THEN 'Storno / Doppelbestellung / falsche Bestellung'
    ...
    END AS subscription_cancellation_reason
     ...
     , subscription_has_recurring_product
FROM cte_3
-- WHERE customer_id = 'K30289455'  -- 3 year contract
-- WHERE customer_id = '30014401050'  -- 3 year contract
-- WHERE customer_id = '30003301581'  -- 1 or 3 year contract ???

;
SELECT * FROM dwh_main.dim_combined_subscription
