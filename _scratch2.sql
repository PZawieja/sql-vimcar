...
-- select * from cte_revenue_1;
   , cte_2 AS (
        ...
        END                                                                                    AS subscription_status
         ...
         , coalesce(
                ...
                AND first_3y_order_dt_contract_lvl + INTERVAL '3 years' > current_date
                             ...
                             ELSE valid_to_dt END::DATE)
                    FILTER ( WHERE contract_status_map = 2 AND valid_to_dt_contract_lvl >= current_date ), valid_to_dt_contract_lvl) AS subscription_end_dt
    ...
    GROUP BY cb.cb_cust_id, customer_id, contract_id, contract_status_map, contract_terminated_dt, valid_to_dt, valid_to_dt_contract_lvl, three_year_contract
)
SELECT
    subscription_id
     , customer_id
     ...
     , CASE
           WHEN ABS(ctr.contract_provisioning_end_date_key - subscription_end_dt) < 6  -- in case there is max 5 days difference between contract end date and calculated subscription_end_dt then we take the contract end date from the Shop
               THEN ctr.contract_provisioning_end_date_key
           ELSE subscription_end_dt
    END AS subscription_end_dt
