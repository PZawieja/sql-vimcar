SELECT extract('day' from '2022-01-15'::DATE)
...
WITH cte_pdb_0 AS (SELECT t.customer_id
                        ...
                        , product_terms
--                         , customer_is_new
--                         , customer_is_new_credit_note_ignored
                        , CASE
                              WHEN customer_is_new = TRUE
                                  -- to capture the new invoices from the additional two months:
                                  OR (ni.is_new_invoice_id IS NOT NULL
                          ...
                          END customer_is_new
                        , CASE
                              WHEN customer_is_new_credit_note_ignored = TRUE
                                  -- to capture the new invoices from the additional two months:
                                  OR (ni.is_new_invoice_id IS NOT NULL
                                      AND invoice_provisioning_start_mth = reporting_month
                                      AND reporting_month BETWEEN t2.customer_is_new_credit_note_ignored_start_mth AND
                                          CASE
                                              WHEN t2.new_mrr_window_mths =2 THEN t2.customer_is_new_credit_note_ignored_start_mth + INTERVAL '2 months'
                                              WHEN t2.new_mrr_window_mths =3 THEN t2.customer_is_new_credit_note_ignored_start_mth + INTERVAL '3 months'
                                        END)
                                  THEN TRUE
                              ELSE FALSE
                        END customer_is_new_credit_note_ignored
--                         , customer_is_new_start_mth
--                         , customer_is_new_start_mth + INTERVAL '2 months'
                        , flag_future
                        , reporting_month
                        , churn_mth
