...
WITH cte_contract_product AS (
    SELECT
        cco.outbound_id::VARCHAR(9) AS shop_contract_id
         ...
         , started                      AS contract_start
         , provisioning_period_start    AS provisioning_start
         , provisioning_period_end      AS provisioning_end
...
--                       AND coalesce(b.comment,'dummy') NOT IN ('Excluded from migration: Ghost customer or internal Vimcar email address.')
    ...
    GROUP BY cco.outbound_id, pmap.cb_plan_id, cbmap.cb_plan_id_map, cbmap.cb_plan_id_map_nbr, items.product_id, item_net_unit_price_amount
           , pmap.cb_addon_id, pmap.cb_addon_price_net, pmap.monthly_payment, cco.started, cco.provisioning_period_start, cco.provisioning_period_end
)
   , cte_invoice_product AS (
    SELECT
        ci.contract_outbound_id::VARCHAR(9) AS shop_contract_id
                  ...
                  , pmap.cb_addon_price_net
                  , pmap.monthly_payment
                  , MIN(provisioning_start) AS contract_start
                  , MAX(provisioning_start) AS provisioning_start
                  , MAX(provisioning_end) AS provisioning_end
...
--    select * from cte_invoice_product;
   , cte_inv_ctr_prod_union AS (
    SELECT i.*, coalesce(c.item_quantity,0) AS item_quantity, 'i' AS flag
    ...
    UNION
    SELECT * , 'c' AS flag FROM cte_contract_product
)
-- select * FROM cte_inv_ctr_prod_union ;
, cte_inv_ctr_prod_dates_fix AS (SELECT shop_contract_id
                                      ...
                                      , MAX(provisioning_start)                       AS provisioning_start
                                      , MAX(provisioning_end)                         AS provisioning_end
                                 FROM cte_inv_ctr_prod_union
                                 GROUP BY shop_contract_id, plan_id, cb_plan_id_map_nbr, unit_price_net, cb_addon_id,
                                          cb_addon_price_net, monthly_payment, item_quantity)
   , unique_subscriptions AS (
    SELECT
        shop_contract_id
         ...
         , provisioning_end
         , SUM(item_quantity) AS item_quantity
    FROM cte_inv_ctr_prod_dates_fix
    GROUP BY shop_contract_id, unique_subscription_id, plan_id, cb_addon_id, cb_addon_price_net, contract_start, provisioning_start, provisioning_end)
...
---
        ...
        END AS "subscription[status]"
         ...
         , s.contract_start AT TIME ZONE 'Europe/Berlin' AS "subscription[started_at]"
         , s.provisioning_start AT TIME ZONE 'Europe/Berlin' AS "subscription[current_term_start]"
         , s.provisioning_end AT TIME ZONE 'Europe/Berlin' AS "subscription[current_term_end]"
         , GREATEST(c.provisioning_period_end AT TIME ZONE 'Europe/Berlin',
                    CASE
                        ...
                        WHEN monthly_payment = FALSE
                            ...
                            THEN s.provisioning_end
                        ELSE s.provisioning_end + ((extract(year from age(date_trunc('month',current_date), date_trunc('month',c.started) + INTERVAL '1 day'))
        ...
        END AS "coupon_ids[0]"
         ...
         , to_char(
            btrim(CASE
                      ...
                      ELSE coalesce(shipping_contact -> 'address' ->> 'postal_code',contact -> 'address' ->> 'postal_code')
...
WHERE shop_contract_id = '77685413'
)
