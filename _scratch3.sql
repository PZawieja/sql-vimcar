SELECT * FROM contract WHERE outbound_id = 'V84845254' ;

;
WITH test AS (
    SELECT
    '2019-08-25 12:00:00'::timestamptz AS started
    , '2021-08-25 12:00:00'::timestamptz AS provisioning_period_start
    , '2021-10-25 12:00:00'::timestamptz AS provisioning_period_end
    , '3-years' AS plan_id
    , FALSE AS monthly_payment
    , 'active' AS status
)
, cte AS (SELECT
         ...
         , GREATEST(c.provisioning_period_end AT TIME ZONE 'Europe/Berlin',
                    CASE
                        WHEN c.plan_id ILIKE '%3-years%'
                            OR c.plan_id ILIKE '%3_years%'
...
"subscription[started_at]"::date
     , "subscription[current_term_start]"::date
     , "subscription[current_term_end]"::date
...
--         END AS billing_cycles  -- CHECK! remaining billing cycles (OLD logic)
        ...
        END AS "subscription[contract_term_billing_cycle_on_renewal]"
         ...
         , NULL AS "create_current_term_invoice"  -- CHECK!
...
-- WHERE shop_contract_id = 'V84845254'
)
