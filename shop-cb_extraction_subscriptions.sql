...
--                               , least("subscription[started_at]","contract_term[contract_start]","subscription[current_term_start]") AS "contract_term[contract_start]" --2022-06-23, contract start date cannot not be lesser than the Subscription start date
...
SELECT "customer[email]"
     ...
     , CASE WHEN "subscription[status]" = 'active' THEN "contract_term[contract_start]" END AS "contract_term[contract_start]"
...
SELECT
    CASE
     ...
     , "subscription[current_term_start_shop]"
     , CASE
           WHEN "subscription[current_term_end]" < current_date
               AND is_monthly_billing_plan = TRUE
               THEN "subscription[current_term_start]" + INTERVAL '1 month'
           WHEN "subscription[current_term_end]" < current_date
               AND is_monthly_billing_plan = FALSE
               THEN "subscription[current_term_start]" + INTERVAL '1 year'
           ELSE "subscription[current_term_start]"
    END AS "subscription[current_term_start]"
     , "subscription[current_term_end]"
     , CASE WHEN "subscription[status]" = 'active' THEN "contract term end" END AS "contract term end"
     , CASE WHEN "subscription[status]" = 'active' THEN "contract_term[contract_start]" END AS "contract_term[contract_start]"
     ...
     , CASE WHEN "subscription[status]" = 'active' THEN billing_cycles END AS billing_cycles
     , CASE WHEN "subscription[status]" = 'active' THEN "contract_term[billing_cycle]" END AS "contract_term[billing_cycle]"
     , CASE WHEN "subscription[status]" IN ('active', 'future') THEN "subscription[contract_term_billing_cycle_on_renewal]" END AS "subscription[contract_term_billing_cycle_on_renewal]"
     , coupon_code_temp
     , cancel_reason_code
     , cf_lifetime_licence
     , FALSE AS "create_current_term_invoice"
...
--          , cbmap.cb_plan_id_map
--          , cbmap.cb_plan_id_map_nbr
         , 'logbook_b2c-yearly-eur-01_08' AS plan_id
         , 'logbook_b2c-yearly-eur-01_08' AS cb_plan_id_map
         , 1 AS cb_plan_id_map_nbr
         , pmap.cb_addon_id
         , pmap.monthly_payment
         , MAX(pmap.cb_addon_price_net) AS cb_addon_price_net
         , MAX(item_net_unit_price_amount) AS unit_price_net
...
--          , cbmap.cb_plan_id_map_nbr
         ...
         , MAX(pmap.cb_addon_price_net) AS cb_addon_price_net
         , MAX(item_net_unit_price_amount) AS unit_price_net
         , MIN(ci.provisioning_start AT TIME ZONE 'Europe/Berlin') AS min_provisioning_start
         , MAX(ci.provisioning_start AT TIME ZONE 'Europe/Berlin') AS max_provisioning_start
         , MAX(ci.provisioning_end AT TIME ZONE 'Europe/Berlin') AS max_provisioning_end
    FROM public.shop_extraction_order_invoice ci
             JOIN public.customer cc
                  ON cc.id = ci.customer_id
--              JOIN public.shop_extraction_current_batch b  -- join batch table
